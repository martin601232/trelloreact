...
const ListaLista = () => {
  ...
  const eliminarTarea = (id) => {
    ...
    axios
      .delete(
        "http://localhost:8000/api/tareas/"+id,config
      )
      .then((res) => {
        loadRespuestaList();
      })
    ...
    <Container>
      <Row>
        ...
        <td>
          ...
          <Link className="btn btn-primary" to={"/add/"+id}>Agregar</Link>
        </td>
        ...
        <Container  className="grid" >
           ...
           // return(
                ...
                return (
                  <Card style={{ width: '18rem'}}  className="box" key={"item-" + item.id}>
                    <Card.Body >
                      <Card.Title>{item.nombre +"  "+ item.orden}</Card.Title>
                      </Card.Body>
                      <ListGroup className='list-group-flush'>
                      {ListaRespuesta.map((item2) => {
                        
                        {
                          if(item2.lista_id== item.id && item2.estado){
                        return(
                        <ListGroupItem>
                          <Card.Body>
                          <Card.Title>{item2.titulo}</Card.Title>
                          <><td>
            <Link className="btn btn-primary" to={'/tarea/'+item2.id+'/'+item.id+'/'+id}>Edit</Link>
          </td><td>
              <Button variant="danger" onClick={() => { eliminarTarea(item2.id); } }>Eliminar</Button>
            </td> 
            <td>
            <Link className="btn btn-primary" to={'/detalle/'+item2.id+'/'+id}>Entrar</Link>
          </td> </>
                            </Card.Body>
                            </ListGroupItem>
                        )
          ...
          return (
            ...
            <td>
            <Link className="btn btn-primary" to={'/tarea/'+item.id+'/'+id}>nuevo</Link>
          </td> </>
          )
        }else {
          return (
